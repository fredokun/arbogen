    ...
    ("-eps2_factor", Arg.Float
     ...
     "<x> : set the refinement factor for epsilon in simple iteration (a strictly positive float number)");
       ...
       (fun x ->
          if x < 0.0 then
            begin
              eprintf "Error: wrong divergence threshold factor parameter %f => must be positive\n...aborting\n%!" x ;
              exit 1;
            end
          else begin
