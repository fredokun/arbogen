...
let init n z =
  {z; values= Array.make n 0.; derivate_values= Array.make n 0.}

let copy oracle =
  {z = oracle.z; values= Array.copy oracle.values; derivate_values= Array.copy oracle.derivate_values}
...
module Eval = struct
  ...
  let deriv_expression oracle =
    let rec aux : int Grammar.expression -> float = function
      | Z n -> if n == 0 then
                 ...
                 (float_of_int n) *. oracle.z ** float_of_int (n - 1)
      | Product (e1, e2) ->
  ...
  let grammar_inplace oracle_dest oracle (grammar : Grammar.t) =
    Array.iteri (fun i r -> oracle_dest.values.(i) <- expression oracle r) grammar.rules;
    Array.iteri (fun i r -> oracle_dest.derivate_values.(i) <- deriv_expression oracle r) grammar.rules
...
module Naive = struct
  ...
  type config = {epsilon1: float; epsilon2: float; epsilon3: float; zstart: float}
  ...
  let search_singularity {epsilon1; epsilon2; zstart; _} grammar =
    ...
    let rec search init_values zmin zmax =
      if zmax -. zmin < epsilon1 then
        (zmin, zmax, iteration_simple grammar {init_values with z=zmin} epsilon2)
      else
  ...
  let search_expectation {epsilon1; epsilon2; epsilon3; zstart} n grammar =
    ...
    let rec search init_values zmin zmax =
      ...
      | Val v ->
         ...
         if abs_float diff  < epsilon3 then
           zmin, zmax, eval
         else
           if diff < 0. then
             let init_values' = init len ((zmin +. init_values.z) /. 2.) in
  ...
  let default_singular_config = {epsilon1= 1e-9; epsilon2= 1e-9; zstart= 0.; epsilon3= 0.}
  let default_expectation_config = {epsilon1= 1e-9; epsilon2= 1e-9; epsilon3= 5.; zstart= 0.}
