...
(** A grammar is an array of production rules.
    For performance reasons, the non-terminals of the grammars are represented
    by integers:
...
and 'ref expression =
  | Z of int
  (** The atom, potentially elevated to the power of some integer.
      This accounts for the size of the generated object. *)
  ...
  (** Sequence of objects. This behaves as a Kleene start, that is
      [Seq e] behaves as [Union (Z 0, Product (e, Seq e)] *)

  | Ref of 'ref
  (** A non-terminal symbol.
      The type of the references is left abstract in order to allow both integer
      references ([Ref i] refers to the [i]th non-terminal of the grammar)
      and strings (for use in the parser). *)
