...
let speclist =
  let set_verbosity n =
    if n < 0 then begin
      ...
      exit 1
    end else Options.globals.verbosity <- n
  in
  ...
  [("-version", Arg.Unit (fun () -> Format.printf "%s@." version_str; exit 0),
   ...
   ("-id", Arg.Unit (fun () -> Options.globals.with_id <- true),
    ": number the nodes");

   ("-typ", Arg.Unit (fun () -> Options.globals.with_type <- true),
    ": show the type of nodes");

   ("-randgen", Arg.String (fun s -> Options.set "randgen" (String s)),
    "[ocaml|randu|randnull] : set the random number generator");

   ("-oracle", Arg.Unit (fun () -> Options.globals.print_oracle <- true),
    ": output an oracle");

   ("-indent", Arg.Unit (fun () -> Options.globals.indent <- true),
    ": indent the output")
  ]
...
let print_tree tree =
  ...
  | 0 -> print arb_printer Options.globals.fileName ".arb"
  | 1 -> print dot_printer Options.globals.fileName ".dot"
  | 2 -> print xml_printer Options.globals.fileName ".xml"
  | 3 ->
    let filename =
      if Options.globals.fileName = ""
      then "tree"
      else Options.globals.fileName
  in
...
let init_rng () =
  ...
  let seed = match Options.globals.random_seed with
    | Some seed -> seed
...
let () =
  Arg.parse speclist
    (fun arg ->
       if Options.globals.grammar_file = ""
       then Options.globals.grammar_file <- arg
  ...
  if Options.globals.print_oracle then begin
